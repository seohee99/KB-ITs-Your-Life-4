...
SELECT ename, sal, CEIL(sal*0.05) "경조비"
FROM emp
WHERE SAL>= 2000;
...
SELECT empno, ename, sal, FLOOR(comm*1.5) "뽀나스"
FROM emp
WHERE comm > 0 ;
...
SELECT ename, hiredate, hiredate+100 "100일 후", sal
FROM emp;


-- 2) 입사일로부터 6개월이 지난 후의 날짜를 구하려고 한다.  입사일, 6개월 후의 날짜, 급여를 출력하라
SELECT ename, hiredate,ADD_MONTHS(hiredate,6)  "6개월 후", sal
FROM emp;

-- 3) 입사한 달의 근무일수를 계산하여 부서번호, 이름, 근무일수를 출력하라.
SELECT deptno,ename,FLOOR( SYSDATE-hiredate) "근무일수"
FROM emp;
...
SELECT ename, TO_CHAR(hiredate, 'YYYY"."MM ') 입사일
FROM emp;

-- 3) 모든 직원의 번호, 이름, 급여를 출력하라. 단, 급여앞에 화폐표시($), 그리고 천단위(,)표시, 소수점이하 2자리가 표시되도록 하라.
SELECT empno, ename, CONCAT('$',TO_CHAR(sal,'FM999,999.00')) 급여
FROM emp;
...
SELECT ename, sal,
        DECODE (comm, null, '없음', comm) comm
FROM emp;

-- 2)  모든 직원의 이름, 급여, 커미션, 연봉을 출력하라. 
-- comm이 null이면 없음으로 기입
-- 보너스는 comm이 null이면 10
SELECT ename, sal,  DECODE (comm, null, '없음', comm) comm, (sal * 12 + DECODE (comm, null,10, comm)) 연봉
FROM emp;
